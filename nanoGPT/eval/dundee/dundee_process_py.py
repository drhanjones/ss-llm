...
def insert_into_db(processed_model_surprisal_df, conn, c):
    ...
    for index, row in processed_model_surprisal_df.iterrows():
        c.execute("INSERT INTO ModelSurprisalScores (ModelID, StoryWordID, SurprisalScore) VALUES (?, ?, ?)", (row["ModelID"], row["StoryWordID"], row["SurprisalScore"]))
...
def get_incomplete_model_ids(conn, c):
    ...
    model_ids = c.execute("SELECT ModelID FROM ModelSurprisalScores GROUP BY ModelID HAVING Count(*) != ? ORDER BY ModelID DESC", (expected_row_counts,)).fetchall()
