...
parser = argparse.ArgumentParser(
    description="Fit surprisal models with specified equation pair"
)
parser.add_argument(
    "--equation_pair_index",
    type=int,
...
if "Darwin" in platform.system():
    ...
    raise NotImplementedError(
        "This code is not intended to be run on pop-os machine yet."
    )
...
def predict_rt(fit_name):
    ...
    ro.r(
        """predicted_rt <- baseline_df[, c("RTUID", "WorkerID", "StoryWordID", "LogRT", "PredictedLogRT")]"""
    )
...
def save_json_lines_predict(model_id, save_jsonl, equation_pair_index=None):
    ...
    os.makedirs(
        f"dundee_predict/models_results_equation_pair_{equation_pair_index}", exist_ok=True
    )
...
model_id_list = sorted(
    pd.read_sql_query(MODEL_LIST_QUERY, conn)["ModelID"].unique().tolist()
)
...
for model_id in tqdm.tqdm(model_id_list):
    ...
    try:
        ...
        predicted_rt_df["ModelID"] = model_id
        predicted_rt_df = predicted_rt_df[["ModelID", "RTUID", "WorkerID", "StoryWordID", "LogRT", "PredictedLogRT"]]
        predicted_rt_df["EquationPairIndex"] = equation_pair_index

        predicted_rt_df_json = predicted_rt_df.to_dict(orient="records")

        save_json_lines_predict(
            model_id, predicted_rt_df_json, equation_pair_index=equation_pair_index
        )
