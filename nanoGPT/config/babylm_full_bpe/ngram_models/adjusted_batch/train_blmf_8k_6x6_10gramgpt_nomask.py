...
if init_from == "resume":
    out_dir = "output_dump/out-babylm_full_bpe_8k-6x6-nomask-6092913-5gramadj"
    wandb_run_id = 'm6vm43sy'
    wandb_run_name = 'babylm_full_bpe_8k_6x6_nomask_gpt2_server_run_6092913_5gramadj'
    assert out_dir is not None, "Need to specify out_dir for resuming training"
    assert wandb_run_id is not None, "Need to specify wandb_run_name for resuming training"
    ...
    out_dir = f'output_dump/out-{dataset}-{lay_x_head}-{mask_part}-{unique_id}-10gramadj'
    wandb_run_name = f'{dataset}_{lay_x_head}_{mask_part}_gpt2_{sysname}_run_{unique_id}_10gramadj'
