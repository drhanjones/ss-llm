...
#curriculum learning
curriculum = True
batchloader = "curriculum"
curriculum_pacing_fn_name = "log"
curriculum_start_percent = 0.02 #When does the curriculum start increasing the difficulty, in  terms of percentage of total steps
curriculum_end_percent = 1 #When does the curriculum reach maximum difficulty, in terms of percentage of total steps
curriculum_start_difficulty = 0.02 #Starting difficulty of the dataset as a percentile of the dataset's difficulty
curriculum_log_growth_rate_c = 20 #Growth rate of the curriculum in logspace
...
if curriculum:
    if curriculum_pacing_fn_name == "log":
        curriculum_part = f"curr_log"
    elif curriculum_pacing_fn_name == "linear":
        curriculum_part = f"curr_lin"

lay_x_head = f'{n_layer}x{n_head}'

torch_seed_default = 46674

unique_id = os.environ.get("SLURM_JOB_ID", str(int(time.time()))) #Using SLURM_JOB_ID as unique id else use time#  #str(int(time.time()))
out_dir = f'output_dump/out-{dataset}-{lay_x_head}-{mask_part}-{curriculum_part}-{unique_id}'
wandb_run_name = f'{dataset}_{lay_x_head}_{mask_part}_{curriculum_part}_gpt2_{sysname}_run_{unique_id}'
